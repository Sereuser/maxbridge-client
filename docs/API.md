# MaxBridge API Documentation

## Обзор

MaxBridge предоставляет полный доступ к API мессенджера MAX через WebSocket соединение. Библиотека поддерживает все основные функции: отправка сообщений, управление чатами, загрузка файлов и многое другое.
